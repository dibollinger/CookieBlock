...
const setupFunctions = {
    "setup_top_names": (source, vector_size, args) => {
        top_names = {};
        getLocalData(browser.extension.getURL(source), "text", (resp) => {
...
getLocalData(browser.extension.getURL("ext_data/features.json"), "json", setupFeatureResourcesCallback);
...
const perCookieFeatures = {
    ...
    "feature_compressed_length_mean_and_stddev":  (sparse, curr_idx, cookie_data, args) => {
        ...
        let cookieUpdates = cookie_data["variable_data"];
        for (let i = 0; i < cookieUpdates.length; i++) {
            let compressed = LZString.compressToUTF16(cookieUpdates[i]["value"]);
